...
/// Mapeia: Req 8, RNF 11, ADR-0008, TASK-02.
/// </summary>
public sealed class ContractValueTests
{
    ...
    {
        var cv = new ContractValue(new Money(1000L, "BRL"), new Money(500L, "BRL"), 12);
    ...
    {
        var cv = new ContractValue(new Money(5000L, "BRL"), Money.Zero("BRL"), 0);
    ...
    {
        var cv = new ContractValue(Money.Zero("BRL"), Money.Zero("BRL"), 0);
    ...
    {
        var act = () => new ContractValue(Money.Zero("BRL"), new Money(100L, "BRL"), 0);
    ...
    {
        var act = () => new ContractValue(Money.Zero("BRL"), Money.Zero("BRL"), -1);
        act.Should().Throw<DomainException>();
    }
    ...
    {
        var act = () => new ContractValue(new Money(-1L, "BRL"), Money.Zero("BRL"), 0);
        act.Should().Throw<DomainException>();
    }
    ...
    {
        var a = new ContractValue(new Money(1000L, "BRL"), new Money(500L, "BRL"), 12);
        var b = new ContractValue(new Money(1000L, "BRL"), new Money(500L, "BRL"), 12);
        a.Should().Be(b);
    }

    [Fact(DisplayName = "ADR-0008: ContractValue preserva Currency")]
    public void ContractValue_PreservesCurrency()
    {
        var cv = new ContractValue(new Money(1000L, "USD"), new Money(500L, "USD"), 12);
        cv.Currency.Should().Be("USD");
        cv.Setup.Currency.Should().Be("USD");
        cv.Mensal.Currency.Should().Be("USD");
    }

    [Fact(DisplayName = "ADR-0008: ContractValue com moedas diferentes lança DomainException")]
    public void ContractValue_MixedCurrencies_ThrowsDomainException()
    {
        var act = () => new ContractValue(new Money(1000L, "BRL"), new Money(500L, "USD"), 12);
        act.Should().Throw<DomainException>();
    }
    ...
    public Property PBT03b_ContractValue_SetupOnly_Total()
    {
        return Prop.ForAll(
            ...
            setupCents =>
            {
                var cv = new ContractValue(new Money(setupCents, "BRL"), Money.Zero("BRL"), 0);
