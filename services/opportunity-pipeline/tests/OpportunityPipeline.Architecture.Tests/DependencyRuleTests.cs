...
/// conforme design §3 do módulo opportunity-pipeline.
///
/// Executa em 100% dos PRs e falha o CI em caso de violação (TASK-01).
///
...
public sealed class DependencyRuleTests
{
    ...
    ///
    /// Domain não deve conhecer EF Core, Npgsql, Redis, GCP SDKs ou qualquer adapter.
    /// Mapeia: design §3, design §2 (P1 — Domain independente de infraestrutura), RNF 11.
    /// </summary>
    ...
    ///
    /// Dependência de Domain em Api criaria ciclo e acoplamento ao web framework.
    /// Mapeia: design §3.
    /// </summary>
    ...
    /// Mapeia: design §3, design §2 (P1), design §5.4 (behaviors).
    /// </summary>
    ...
    ///
    /// Contracts é a camada mais externa (pública) — sem dependências internas.
    /// Mapeia: design §3.
    /// </summary>
    ...
    ///
    /// Nenhum adapter de banco, Redis ou GCP pode aparecer em Contracts.
    /// Mapeia: design §3.
    /// </summary>
    ...
    /// <summary>
    /// Regra D: Domain não referencia namespaces de EF Core, Npgsql, Redis,
    /// GCP SDKs, ASP.NET Core ou MediatR.
    ///
    /// Garante que o domínio permanece puro e testável sem dependência de framework.
    /// Crucial para os cálculos financeiros em centavos (RNF 11) e para PBTs serem
    /// executáveis sem infraestrutura.
    /// Mapeia: design §2 (P1, P2, P6), design §3.
    /// </summary>
    ...
    public void Domain_ShouldNotContain_InfrastructureNamespaces()
    {
        ...
        result.IsSuccessful.Should().BeTrue(
            because: "Domain não deve referenciar namespaces de infraestrutura " +
                     ...
                     "— domínio deve ser puro e testável sem framework (design §2, §3, RNF 11). " +
                     $"Tipos em violação: [{ArchitectureRules.FormatFailingTypes(result)}]");
    }
}
